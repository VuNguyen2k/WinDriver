...
KSPIN_LOCK m_DataLock;
UserDataBuffer m_UserDataBuffer;
...
--*/
    ...
    ntStatus = m_UserDataBuffer.init();
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("User Data Buffer init failed, 0x%x", ntStatus)),
        Done);
    KeInitializeSpinLock(&m_DataLock);
...
)
{
    ...
    unsigned int data_length = pLocation->Parameters.DeviceIoControl.InputBufferLength;

    KIRQL old;

    float new_data[] = { 5, 6, 7, 8 };
    ...
    case IOCTL_PUSH_DATA:
        ...
        ntStatus = m_UserDataBuffer.put(user_data, data_length);
