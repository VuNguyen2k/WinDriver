...
#define USERDATABUFFER_POOLTAG 'FFUB'

NTSTATUS UserDataNode::init(float* user_data, unsigned int data_length)
{
    float* _data = NULL;
    if (user_data != NULL)
    {
        _data = (float*)ExAllocatePool2(POOL_FLAG_NON_PAGED, data_length, USERDATABUFFER_POOLTAG);
        if (_data == NULL) return STATUS_MEMORY_NOT_ALLOCATED;

        RtlCopyMemory(_data, user_data, data_length);
    }

    this->data = _data;
    this->size = data_length / sizeof(float);
    this->read_head = 0;

    return STATUS_SUCCESS;
}

NTSTATUS UserDataNode::update(float* user_data, unsigned int data_length)
{
    if (user_data != NULL)
    {
        RtlCopyMemory(this->data, user_data, data_length);
    }
...
NTSTATUS UserDataNode::get_data_byte(BYTE* output, unsigned int requested_bytes)
{
    long* outputl = reinterpret_cast<long*>(output);
    UserDataNode* pCurrent = this;

    unsigned int node_count = 1; // will fill 0 if this value reach buffer->size;

    for (unsigned int i = 0; i < requested_bytes / sizeof(float);)
    {
        if (pCurrent->readable())
            outputl[i] = (long)(pCurrent->data[pCurrent->read_head] * 2147483647);
        else
        {
            if (node_count >= this->buffer->size)
            {
                outputl[i] = 0;
                i++;
            }
            ...
            {
                pCurrent = &(this->buffer->pNodes[(this->buffer->head_index + node_count) % this->buffer->size]);
                node_count++;
            }
        ...
        i++, pCurrent->read_head++;
    }

    return STATUS_SUCCESS;
}

NTSTATUS UserDataNode::get_data_float(float* output, unsigned int requested_bytes)
{
    UserDataNode* pCurrent = this;
    ...
    {
        if (pCurrent->readable())
            output[i] = pCurrent->data[pCurrent->read_head];
        else
        {
            if (node_count >= this->buffer->size)
            {
...
bool UserDataNode::readable()
{
    return (this->read_head < this->size);
}

bool UserDataNode::data_inserted()
{
    return (this->data != NULL);
}


NTSTATUS UserDataBuffer::init(unsigned int cache_size)
{
    this->size = cache_size;
    this->pNodes = (UserDataNode*)ExAllocatePool2(POOL_FLAG_PAGED, this->size * sizeof(UserDataNode), USERDATABUFFER_POOLTAG);
    if (this->pNodes == NULL)
        return STATUS_MEMORY_NOT_ALLOCATED;

    for (unsigned int i = 0; i < this->size; i++)
    {
        this->pNodes[i].init(NULL, 0);
        this->pNodes[i].buffer = this;
    }

    return STATUS_SUCCESS;
}

NTSTATUS UserDataBuffer::uninit()
{
    ExFreePoolWithTag(this->pNodes, USERDATABUFFER_POOLTAG);
    this->pNodes = NULL;

    return STATUS_SUCCESS;
}

NTSTATUS UserDataBuffer::put(float* data, unsigned int data_length)
{
    UserDataNode* pCurrent = &(this->pNodes[this->head_index]);
    if (pCurrent->data_inserted())
        pCurrent->update(data, data_length);
    else pCurrent->init(data, data_length);
    this->seek();
...
NTSTATUS UserDataBuffer::get_data_byte(BYTE* output, unsigned int requested_bytes)
{
    UserDataNode* pCurrent = &(this->pNodes[this->head_index]);
    return pCurrent->get_data_byte(output, requested_bytes);
}

NTSTATUS UserDataBuffer::get_data_float(float* output, unsigned int requested_bytes)
{
    UserDataNode* pCurrent = &(this->pNodes[this->head_index]);
    return pCurrent->get_data_float(output, requested_bytes);
}