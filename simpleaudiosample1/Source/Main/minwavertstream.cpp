...
extern KSPIN_LOCK m_DataLock;
extern UserDataBuffer m_UserDataBuffer;
...
NTSTATUS UserDataNode::init(float* user_data, unsigned int data_length)
{
    ...
    if (user_data != NULL)
    {
        _data = (float*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(float) * data_length, USERDATABUFFER_POOLTAG);
...
NTSTATUS UserDataNode::get_data(BYTE* output, unsigned int requested_bytes)
{
    long* outputl = reinterpret_cast<long*>(output);

    UserDataNode* pCurrent = this;
    for (unsigned int i = 0; i < requested_bytes;)
    {
        if (pCurrent->readable()) outputl[i] = (long)(pCurrent->data[pCurrent->read_head] * _I32_MAX);
        else
        {
        ...
        if (pCurrent->read_head < pCurrent->size)
            pCurrent->read_head++;
        i++;
    }
...
NTSTATUS UserDataBuffer::init()
{
    this->pHead = (UserDataNode*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(UserDataNode), USERDATABUFFER_POOLTAG);
    if (this->pHead == NULL) return STATUS_MEMORY_NOT_ALLOCATED;

    this->pHead->init(NULL, 0);
...
NTSTATUS UserDataBuffer::uninit()
{
    ...
    UserDataNode* pCurrent = this->pHead;
    while (pCurrent != NULL)
    {
        UserDataNode* pNext = pCurrent->pNext;
        pCurrent->uninit();
        ExFreePoolWithTag(pCurrent, USERDATABUFFER_POOLTAG);
        pCurrent = pNext;
    }
...
NTSTATUS UserDataBuffer::put(float* data, unsigned int data_length)
{
    ...
    if (this->size > 10)
    {
        UserDataNode* pNext = this->pHead->pNext;
        if (pNext != NULL)
        {
            this->pHead->uninit();
            ExFreePoolWithTag(this->pHead, USERDATABUFFER_POOLTAG);
            this->pHead = pNext;
    ...
    UserDataNode* pNew = (UserDataNode*)ExAllocatePool2(POOL_FLAG_NON_PAGED, sizeof(UserDataNode), USERDATABUFFER_POOLTAG);
    if (pNew == NULL) return STATUS_MEMORY_NOT_ALLOCATED;

    pNew->init(data, data_length);
...
NTSTATUS UserDataBuffer::get_data(BYTE* output, unsigned int requested_bytes)
{
    this->pHead->get_data(output, requested_bytes);
    while (!this->pHead->readable())
    {
        UserDataNode* pNext = this->pHead->pNext;
        if (pNext != NULL)
        {
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        KIRQL old;
        KeAcquireSpinLock(&m_DataLock, &old);
        m_UserDataBuffer.get_data(m_pDmaBuffer + bufferOffset, runWrite);
        KeReleaseSpinLock(&m_DataLock, old);
