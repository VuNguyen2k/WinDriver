...
public:
    ...
    NTSTATUS init(float* data, unsigned int size); // size in bytes
    NTSTATUS update(float* data, unsigned int size);
    NTSTATUS get_data_byte(BYTE* output, unsigned int requested_bytes);
    NTSTATUS get_data_float(float* output, unsigned int requested_bytes);
    NTSTATUS uninit();
};
...
public:
    void seek() { this->head_index = (this->head_index + 1) % this->size; }

    NTSTATUS init(unsigned int cache_size);
    NTSTATUS put(float* data, unsigned int length);
    NTSTATUS get_data_byte(BYTE* data, unsigned int requested_byte);
    NTSTATUS get_data_float(float* data, unsigned int requested_byte);
    NTSTATUS uninit();
};
...
extern KSPIN_LOCK m_MicDataLock;
extern UserDataBuffer m_MicUserDataBuffer;

extern KSPIN_LOCK m_SpeakerDataLock;
extern UserDataBuffer m_SpeakerUserDataBuffer;
extern float* speaker_cachef;
extern long* speaker_cachel;
