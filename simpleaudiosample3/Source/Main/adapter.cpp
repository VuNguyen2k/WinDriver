...
KSPIN_LOCK m_MicDataLock;
UserDataBuffer m_MicUserDataBuffer;

KSPIN_LOCK m_SpeakerDataLock;
UserDataBuffer m_SpeakerUserDataBuffer;
float* speaker_cachef = NULL;
long* speaker_cachel = NULL;

#define IOCTL_PUSH_DATA CTL_CODE(FILE_DEVICE_UNKNOWN, 0x2049, METHOD_BUFFERED, FILE_ANY_ACCESS)
#define IOCTL_GET_DATA  CTL_CODE(FILE_DEVICE_UNKNOWN, 0x2047, METHOD_BUFFERED, FILE_ANY_ACCESS)
UNICODE_STRING DEVICE_NAME = RTL_CONSTANT_STRING(L"\\Device\\DataTransferDevice");
UNICODE_STRING DEVICE_SYMBOLIC_NAME = RTL_CONSTANT_STRING(L"\\??\\DataTransferLink");
...
--*/
{
    ...
    m_MicUserDataBuffer.uninit();
    m_SpeakerUserDataBuffer.uninit();

    if (speaker_cachef != NULL)
    {
        ExFreePoolWithTag(speaker_cachef, MINADAPTER_POOLTAG);
        speaker_cachef = NULL;
    }

    if (speaker_cachel != NULL)
    {
        ExFreePoolWithTag(speaker_cachel , MINADAPTER_POOLTAG);
        speaker_cachel = NULL;
    }
...
--*/
    ...
    DriverObject->MajorFunction[IRP_MJ_CREATE] = MajorFunction;
    DriverObject->MajorFunction[IRP_MJ_CLOSE] = MajorFunction;

    DriverObject->MajorFunction[IRP_MJ_PNP] = PnpHandler;
    DriverObject->MajorFunction[IRP_MJ_DEVICE_CONTROL] = DataHandler;
    ...
    ntStatus = m_MicUserDataBuffer.init(25);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("User Data Buffer init failed, 0x%x", ntStatus)),
        Done);
    KeInitializeSpinLock(&m_MicDataLock);

    ntStatus = m_SpeakerUserDataBuffer.init(24);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("User Data Buffer init failed, 0x%x", ntStatus)),
        Done);
    KeInitializeSpinLock(&m_SpeakerDataLock);

    //
    // Create device
    //
    ntStatus = IoCreateDevice(DriverObject, 0, &DEVICE_NAME, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN, FALSE, &DriverObject->DeviceObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("IoCreateDevice failed, 0x%x", ntStatus)),
        Done);
...
#pragma code_seg("PAGE")
#define IOCTL_TEST		CTL_CODE(FILE_DEVICE_UNKNOWN, 0x2048, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
)
{
    ...
    PIO_STACK_LOCATION pLocation = IoGetCurrentIrpStackLocation(_Irp);

    float* user_data = (float*)_Irp->AssociatedIrp.SystemBuffer;
    unsigned int in_data_length = pLocation->Parameters.DeviceIoControl.InputBufferLength;
    unsigned int out_data_length = pLocation->Parameters.DeviceIoControl.OutputBufferLength;

    KIRQL old;
    NTSTATUS ntStatus = STATUS_SUCCESS;

    switch (pLocation->Parameters.DeviceIoControl.IoControlCode)
    {
    case IOCTL_PUSH_DATA:

        KeAcquireSpinLock(&m_MicDataLock, &old);
        ntStatus = m_MicUserDataBuffer.put(user_data, in_data_length);
        KeReleaseSpinLock(&m_MicDataLock, old);
    ...
    case IOCTL_GET_DATA:
        ...
        KeAcquireSpinLock(&m_SpeakerDataLock, &old);
        ntStatus = m_SpeakerUserDataBuffer.get_data_float(user_data, out_data_length);
        KeReleaseSpinLock(&m_SpeakerDataLock, old);

        _Irp->IoStatus.Information = out_data_length;
        _Irp->IoStatus.Status = ntStatus;
        IoCompleteRequest(_Irp, IO_NO_INCREMENT);

        break;
    /*case IOCTL_TEST:
        RtlCopyMemory(_Irp->AssociatedIrp.SystemBuffer, new_data, 4 * sizeof(float));
        _Irp->IoStatus.Information = 4 * sizeof(float);
