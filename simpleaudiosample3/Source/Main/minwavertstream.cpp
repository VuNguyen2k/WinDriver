...
)
{
    ...
    else
    {
        ...
        if (!g_DoNotCreateDataFiles) // enable this first
        {
...
--*/
{
    ...
    while (ByteDisplacement > 0)
    {
        ...
        m_MicUserDataBuffer.get_data_byte(m_pDmaBuffer + bufferOffset, runWrite);
...
--*/
{
    ULONG bufferOffset = m_ullLinearPosition % m_ulDmaBufferSize;
    KIRQL old;
    ULONG cache_offset = 0;

    DPF(D_VERBOSE, ByteDisplacement);

    // Normally this will loop no more than once for a single wrap, but if
    // many bytes have been displaced then this may loops many times.
    KeAcquireSpinLock(&m_SpeakerDataLock, &old);
    if (speaker_cachef == NULL) // assume that every byte replacement size are same
    {
        speaker_cachef = (float*)ExAllocatePool2(POOL_FLAG_NON_PAGED, ByteDisplacement, MINADAPTER_POOLTAG);
    }

    if (speaker_cachel == NULL) // assume that every byte replacement size are same
    {
        speaker_cachel = (long*)ExAllocatePool2(POOL_FLAG_NON_PAGED, ByteDisplacement, MINADAPTER_POOLTAG);
    }//*/
    ...
    while (ByteDisplacement > 0)
    {
        ...
        RtlCopyMemory(speaker_cachel + cache_offset, m_pDmaBuffer + bufferOffset, runWrite);
        //m_SaveData.WriteData(m_pDmaBuffer + bufferOffset, runWrite);

        bufferOffset = (bufferOffset + runWrite) % m_ulDmaBufferSize;
        ByteDisplacement -= runWrite;
        
        cache_offset += runWrite;
    }

    //m_SaveData.WriteData((BYTE*)speaker_cachel, ByteDisplacement);

    for (int i = 0; i < ByteDisplacement / sizeof(long); i++)
        speaker_cachef[i] = speaker_cachel[i] / 2147483647.f;

    if (speaker_cachef != NULL) m_SpeakerUserDataBuffer.put(speaker_cachef, ByteDisplacement);
